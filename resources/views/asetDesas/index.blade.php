...
<div class="btn-group">
    ...
    <div class="dropdown-menu">
      ...
      <a class="dropdown-item" href="#">Word</a>
        ...
        {{-- <a href="/asetDesa" class="btn btn-success my-3" target="_blank">EXPORT EXCEL</a> --}}
            ...
            <button type="button"  href="{{ route('asetDesas.create') }}" class="btn btn-success" data-toggle="modal" data-target="#exampleModal">
                +
            </button>
   ...
   <br>
    ...
    <table class="table table-bordered ">
        ...
        $i = 0;
        @endphp
        @foreach ($asetDesas as $asetDesa)
        <tr>


            <td>{{ ++$i }}</td>
            <td>{{ $asetDesa->komoditi}}</td>
            <td>{{ $asetDesa->kategori}}</td>
            <td>{{ $asetDesa->keterangan}}</td>
            <td>
                <form onsubmit="return confirm('Apakah anda yakin ingin menghapus {{$asetDesa->komoditi}} ?');"  action="{{ route('asetDesas.destroy',$asetDesa->id_aset_desa) }}" method="POST">

                    {{-- <a class="btn btn-info" href="{{ route('asetDesas.show',$asetDesa->id_aset_desa) }}">Show</a> --}}
    
                    <a class="btn btn-primary" href="{{ route('asetDesas.edit',$asetDesa->id_aset_desa) }}" ><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pencil-square" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
